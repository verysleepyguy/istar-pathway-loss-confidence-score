...
from utils import read_lines, read_string, load_pickle, save_pickle, save_tsv, write_lines
...
class ForwardSumModel(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        # Reconstruction loss: y_mean is either gene expression or, when
        # --pathways is provided, pathway activity scores.
        # TODO: try l1 loss
...
def load_pathway_gene_sets(pathways_file):
    df = pd.read_csv(pathways_file, sep='\t', header=None, usecols=[0, 1])
    df.columns = ['pathway', 'gene']
    df = df.dropna()
    pathway_genes = {}
    for pathway, group in df.groupby('pathway', sort=False):
        genes = group['gene'].astype(str).drop_duplicates().to_list()
        pathway_genes[str(pathway)] = genes
    return pathway_genes


def score_pathways(cnts, pathways_file, method='mean'):
    pathway_genes = load_pathway_gene_sets(pathways_file)
    if len(pathway_genes) == 0:
        raise ValueError(f'No pathway-gene pairs found in {pathways_file}')

    # First-version pathway scoring: min-max normalize each gene in cnts.tsv,
    # then aggregate matched pathway genes per spot. This function is the
    # modular swap point for future GSVA/ssGSEA/AUCell scoring.
    cnts_norm = cnts.astype(np.float32).copy()
    cnts_norm -= cnts_norm.min(0)
    cnts_norm /= (cnts_norm.max(0) - cnts_norm.min(0)) + 1e-12

    scores = {}
    matched = {}
    available = set(cnts_norm.columns)
    for pathway, genes in pathway_genes.items():
        genes_matched = [gene for gene in genes if gene in available]
        if len(genes_matched) == 0:
            print(f'Skipping pathway with no matched genes: {pathway}')
            continue
        vals = cnts_norm[genes_matched]
        if method == 'sum':
            scores[pathway] = vals.sum(1)
        elif method == 'mean':
            scores[pathway] = vals.mean(1)
        else:
            raise ValueError(f'Unknown pathway scoring method: {method}')
        matched[pathway] = genes_matched

    if len(scores) == 0:
        raise ValueError(
                'No pathway genes matched genes in cnts.tsv. '
                'Check gene name capitalization and identifiers.')

    print(
            f'Computed {len(scores)} pathway targets from '
            f'{sum(len(v) for v in matched.values())} matched gene entries.')
    return pd.DataFrame(scores, index=cnts.index), matched
...
def get_data(prefix, pathways=None):
    # cnts.tsv is loaded here as the original spots x genes expression matrix.
    gene_names = read_lines(f'{prefix}gene-names.txt')
    cnts = get_gene_counts(prefix)
    if pathways is None:
        cnts = cnts[gene_names]
    else:
        # Target matrix is changed here from spots x genes to spots x pathways.
        cnts, matched = score_pathways(cnts, pathways)
        pathway_names = cnts.columns.to_list()
        write_lines(pathway_names, f'{prefix}pathway-names.txt')
        write_lines(
                [sanitize_output_name(name) for name in pathway_names],
                f'{prefix}pathway-file-names.txt')
        write_lines(
                [
                    f'{pathway}\t{gene}'
                    for pathway, genes in matched.items()
                    for gene in genes
                ],
                f'{prefix}pathway-matched-genes.tsv')
...
def predict(
        ...
        output_dir='cnts-super', embeddings_file='embeddings-gene.pickle',
        file_name_list=None,
        device='cuda'):
    ...
    for idx_grp in idx_groups:
        ...
        file_name_grp = file_name_list[idx_grp]
        ...
        y_grp = np.concatenate([
            ...
            for z_states in z_states_batches])
        for i, name in enumerate(file_name_grp):
            # Predicted pathway maps are saved to cnts-pathway-super/ in
            # pathway mode, leaving the original cnts-super/ gene output alone.
            save_pickle(y_grp[..., i], f'{prefix}{output_dir}/{name}.pickle')


def predict_calibration_spots(
        model_states, embs, locs, radius, spot_index, y_range, device='cuda'):
    mask = get_disk_mask(radius)
    x = get_patches_flat(embs, locs, mask)
    isin = np.isfinite(x).all((-1, -2))
    x = x[isin]
    out = np.full((len(spot_index), y_range.shape[0]), np.nan, dtype=np.float32)
    if len(x) == 0:
        return out
    ...
    for model in model_states:
        ...
        with torch.no_grad():
            # Simple calibration export: predict pathway activity on the same
            # measured spots used for training. Future versions should use
            # held-out calibration folds to avoid double dipping.
            y = model.forward(x).mean(-2)
        y = y.cpu().detach().numpy()
        y *= y_range[:, 1] - y_range[:, 0]
        y += y_range[:, 0]
        preds.append(y)
    out[isin] = np.median(preds, 0)
    return out


def load_prediction_maps(prefix, output_dir, names, file_names):
    maps = [
            load_pickle(
                f'{prefix}{output_dir}/{file_name}.pickle',
                verbose=False)
            for file_name in file_names]
    maps = np.stack(maps, -1)
    row_ids = [
            f'{i}_{j}'
            for i in range(maps.shape[0])
            for j in range(maps.shape[1])]
    values = maps.reshape((-1, maps.shape[-1]))
    return pd.DataFrame(values, index=row_ids, columns=names)


def export_pathway_confidence_inputs(
        prefix, true_pathways, cal_pred, names, file_names, output_dir):
    # Spot-level confidence debugging/export mode for pathway_confidence.py.
    # Calibration and test predictions intentionally use the same measured
    # spots and the same spot-level prediction function so their scales match.
    # This is not yet full UTOPIA: a future version should use held-out
    # calibration/test folds or aggregate full-resolution predictions back to
    # spot scale before producing calibrated full-resolution confidence maps.
    names = list(names)
    out_dir = f'{prefix}pathway-confidence-inputs/'
    save_tsv(true_pathways, f'{out_dir}calibration_true_pathways.tsv')
    cal_pred = pd.DataFrame(
            cal_pred, index=true_pathways.index, columns=true_pathways.columns)
    save_tsv(cal_pred, f'{out_dir}calibration_predicted_pathways.tsv')

    # First corrected version: use the same spot-level predictions for test
    # input instead of loading differently scaled cnts-pathway-super/ maps.
    test_pred = cal_pred.copy()
    save_tsv(test_pred, f'{out_dir}test_predicted_pathways.tsv')
    write_lines(names, f'{out_dir}pathway_names.txt')

    print('Confidence input export sanity checks:')
    print('  calibration true shape:', true_pathways.shape)
    print('  calibration predicted shape:', cal_pred.shape)
    print('  test predicted shape:', test_pred.shape)
    print(
            '  calibration true mean/max:',
            float(np.nanmean(true_pathways.to_numpy())),
            float(np.nanmax(true_pathways.to_numpy())))
    print(
            '  calibration predicted mean/max:',
            float(np.nanmean(cal_pred.to_numpy())),
            float(np.nanmax(cal_pred.to_numpy())))
    print(
            '  test predicted mean/max:',
            float(np.nanmean(test_pred.to_numpy())),
            float(np.nanmax(test_pred.to_numpy())))
...
def impute(
        ...
        output_dir='cnts-super', embeddings_file='embeddings-gene.pickle',
        file_names=None, state_dir='states',
        export_confidence_inputs=False):

    cnts_df = cnts.copy()
    spot_index = cnts.index
    names = cnts.columns
    cnts = cnts.to_numpy()
    ...
    kwargs_list = [
            dict(
                ...
                prefix=f'{prefix}{state_dir}/{i:02d}/',
                load_saved=load_saved, device=device)
    ...
    cal_pred = None
    if export_confidence_inputs:
        cal_pred = predict_calibration_spots(
                model_states=model_list, embs=embs, locs=locs, radius=radius,
                spot_index=spot_index, y_range=np.stack([cnts_min, cnts_max], -1),
                device=device)
    ...
    predict(
            ...
            embeddings_file=embeddings_file, file_name_list=file_names,
            device=device)
    if export_confidence_inputs:
        export_pathway_confidence_inputs(
                prefix=prefix, true_pathways=cnts_df, cal_pred=cal_pred,
                names=names, file_names=file_names, output_dir=output_dir)
...
def get_args():
    ...
    parser.add_argument(
            '--pathways', type=str, default=None,
            help='Optional long-format pathway file: pathway_name<TAB>gene_name')
    parser.add_argument(
            '--export-confidence-inputs', action='store_true',
            help=(
                'In pathway mode, export wide TSV matrices for '
                'pathway_confidence.py after prediction.'))
...
def main():
    args = get_args()
    embs, cnts, locs = get_data(args.prefix, pathways=args.pathways)
    ...
    is_pathway_mode = args.pathways is not None
    output_dir = 'cnts-pathway-super' if is_pathway_mode else 'cnts-super'
    state_dir = 'states-pathway' if is_pathway_mode else 'states'
    embeddings_file = (
            'embeddings-pathway.pickle'
            if is_pathway_mode else 'embeddings-gene.pickle')
    file_names = None
    if is_pathway_mode:
        file_names = np.array(read_lines(f'{args.prefix}pathway-file-names.txt'))
    ...
    impute(
            ...
            output_dir=output_dir, embeddings_file=embeddings_file,
            file_names=file_names, state_dir=state_dir,
            export_confidence_inputs=(
                args.export_confidence_inputs and is_pathway_mode))
