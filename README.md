...
### Optional Pathway Confidence Scoring

After running pathway-iSTAR prediction, `pathway_confidence.py` can compute
UTOPIA-inspired post-hoc conformal p-values, Benjamini-Hochberg adjusted
p-values, and confidence scores for pathway activity calls. This step is
optional and does not change pathway prediction, model training, or output
folders.

```bash
python pathway_confidence.py \
  --cal-true path/to/calibration_true_pathways.tsv \
  --cal-pred path/to/calibration_predicted_pathways.tsv \
  --test-pred path/to/test_predicted_pathways.tsv \
  --pathway-names path/to/pathway_names.txt \
  --out-dir path/to/confidence_outputs \
  --threshold 0.05
```

For cluster-stratified calibration, also provide matching one-column cluster
label TSV files:
...
python pathway_confidence.py \
  ...
  --pathway-names path/to/pathway_names.txt \
  --out-dir path/to/confidence_outputs \
  --cal-clusters path/to/calibration_clusters.tsv \
  --test-clusters path/to/test_clusters.tsv \
  --min-null 10
```
